 ...
 */
@Import(library = "${assets.path}/mixins/delegate/delegate.js")
public class Delegate {
	...
	/**
	 * Javascript anonymous function called by the event. The signature is function(event,ui,url) where event and ui are
	 * described by jQuery bind. The url parameter is structure representing the Tapestry event url and contains the url
	 * and a function to add a context. See examples below.
	 */
	@Parameter(defaultPrefix = BindingConstants.LITERAL)
	...
	/**
	 * Javascript anonymous function called by the event. Used to manage history on the browser. Signature is
	 * history(event,ui,url). The parameter types are identical to callback.
	 */
	@Parameter(defaultPrefix = BindingConstants.LITERAL)
	private String history;

	/**
	 * Name of the jQuery event. By subclassing bind you can create mixins named by the jQuery event type. This allows
	 * more than one event per element.
	 */
	@Parameter(defaultPrefix = BindingConstants.LITERAL)
	...
	String element() {
		...
		return "#".concat(clientElement.getClientId());
	}
	...
	Link createLink(String event, String marker) {
		ComponentResources parent = resources.getContainerResources().getContainerResources();
		Object[] o = context;
		if (callback != null) {
			if (o == null) {
				o = new Object[1];
	...
	public void afterRender() {
		...
		javaScriptSupport.addInitializerCall("jqdelegate", spec);
		if (doImports) {
			if (zoneUpdate != null) {
