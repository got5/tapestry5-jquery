(function( $ ) {
	$.extend(Tapestry.Initializer, {
		jqdelegate: function(specs) {
			...
			$(el).delegate(specs.selector, specs.eventType, function(event,ui) {
				if ( preventDefault ) {
				...
				if ( u.url ) {
					if ( zoneId ) {
						zoneElement.tapestryZone("update",{url : u.url, params:ui});
					} else {
						$.ajax({url: u.url, params:ui});
					}
				}
			});
		}
