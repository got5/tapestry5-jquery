	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				var self = $(this), data = self.data('fileuploader');
				...
				if ( ! data ) {
					try{
						...
						self.data('fileuploader', uploader);
					}catch(ex){
						$.error( 'Error on init jQuery.fileuploader: ' + ex);
					}
				}
			});
		}
	...
	_error: function(code, fileName){
		...
		qq.extend(qq.FileUploader.prototype, {
			...
			_find: function(parent, type){
				var element = $(parent).find('.' + this._options.classes[type]).get(0);
